...
#### BetOnline 
The [BetOnline poker app](https://www.betonline.ag/poker) is required to run this application in live game mode. You can test the application without the BetOnline software by using the screenshots saved in the "Sample Screenshots" folder.
...
### Screenshots Mode
This mode allows you to run the application based on screenshots taken during a previously played game of [Texas hold'em](https://en.wikipedia.org/wiki/Texas_hold_%27em) played on the [BetOnline poker app](https://www.betonline.ag/poker). It processes data, saves that data to the database and displays relevant data when it is the player of interest's turn to act (the player of interest is the player you are simulating playing as). This is useful for testing the app and/or seeing how it works. A set of screenshots have been included in the "Sample Screenshots" folder in the root of the GitHub directory. As you move through consecutive screenshots you will see the screenshot displayed in the Control window as well as  the game status and the actions taking place. The game status and game actions are also saved to the database.
